...
#   # "cproductos", "ctarjeta_debito", "ctarjeta_visa", "ctarjeta_master", 
#   # "ccaja_ahorro", "mcaja_ahorro", "mprestamos_personales", "cprestamos_personales", "cplazo_fijo", 
#   "mcuentas_saldo", "mautoservicio", 
#   # Transacciones
#   "ctarjeta_debito_transacciones", "ctarjeta_visa_transacciones", "mtarjeta_visa_consumo",
...
#   # Pagos y Cargos - Servicios y Gastos
#   "cpayroll_trx", "mpayroll", "mcomisiones_mantenimiento",
#   # Uso de Tecnología
#   "internet", "tmobile_app", "thomebanking", "chomebanking_transacciones",
#   # Otras variables
#   "Master_mlimitecompra", "Visa_mlimitecompra", "Master_msaldototal", "Visa_msaldototal",
#   # Variables creadas
...
# Código para evaludar k-óptimos (no incluido originalmente en el código)


# Cálculo de la suma de errores cuadráticos (SSE) para diferentes números de clusters
sse <- numeric(10)

# Ejecutar el hclust una sola vez, antes del ciclo
distancia <- dist(dchico[, campos_cluster, with = FALSE])  # Cálculo de la distancia

for (k in 1:10) {
  hclust.rf <- hclust(distancia)  # Generar el objeto hclust
  rf.cluster <- cutree(hclust.rf, k)  # Corta el árbol en k clusters
  
  # Calcular SSE para el número actual de clusters
  sse[k] <- sum(sapply(unique(rf.cluster), function(cluster) {
    cluster_data <- dchico[rf.cluster == cluster, campos_cluster, with = FALSE]
    cluster_center <- colMeans(cluster_data)
    sum(rowSums((cluster_data - cluster_center)^2))  # Sumar los errores cuadráticos
  }))
}

# Gráfico del método del codo
pdf("metodo_del_codo.pdf")
plot(1:10, sse, type = "b", pch = 19, xlab = "Número de clusters", ylab = "SSE", 
     main = "Método del Codo", ylim = c(max(sse), min(sse)))  # Ajustar el eje y
dev.off()

# 
# 
# 
# # Cálculo del Silhouette Score para diferentes números de clusters
# silhouette_scores <- numeric(10)
# 
# for (k in 2:10) {  # Comienza en 2 porque no tiene sentido para 1 cluster
#   rf.cluster <- cutree(hclust.rf, k)
#   silhouette_scores[k] <- mean(silhouette(as.dist(1.0 - modelo$proximity), rf.cluster)$sil_width)
# }
# 
# # Gráfico del Silhouette Score
# pdf("silhouette_score.pdf")
# plot(2:10, silhouette_scores[2:10], type = "b", pch = 19, xlab = "Número de clusters", ylab = "Silhouette Score", main = "Silhouette Score")
# dev.off()
...
# # ejemplo
# dwalkingdead[numero_de_cliente==1550236937, list( numero_de_cliente, foto_mes, periodo ) ]C
